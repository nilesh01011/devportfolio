                ...
                companyName.map((ele) => {
                    ...
                    return (
                        <div key={id} className='w-full flex items-center justify-center flex-col'>
                            ...
                            <motion.h2 variants={textVariant(0.9)} className='xs:text-[32px] xsxs:text-[28px] text-[24px] font-bold xs:mt-[1.5rem] mt-[0.8rem] flex items-center text-center sm:gap-[10px] group'>{company} <Link href={links} target='_blank' title={links} aria-label={`Read more about ${company}`} className='text-[#037ADE] cursor-pointer transition-all group-hover:block hidden hover:underline'>#</Link></motion.h2>
